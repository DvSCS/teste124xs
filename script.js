...
const ffmpeg = createFFmpeg({
    ...
    progress: (p) => {
        console.log('Progresso:', Math.round(p.progress * 100));
    }
});
...
(async () => {
    try {
        ...
        // Adicione esta linha para verificar se o FFmpeg está realmente carregado
        if (ffmpeg.isLoaded()) {
        ...
        errorMessage += '1. Se você está usando HTTPS ou localhost\n';
        errorMessage += '2. Se você está usando um servidor web (como Live Server)\n';
        errorMessage += '3. Se seu navegador está atualizado\n';
        errorMessage += '4. Se você permitiu o acesso a recursos compartilhados\n';
